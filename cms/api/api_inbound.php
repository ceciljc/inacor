 ...
 */
$aColumns = array('sub_judul','judul', 'waktu','content','id','kategori',"status_display");
...
while ($aRow = $DB->fetch_array($rResult)) {
	 ...
	 $qry_region=$DB->query("select kategori from kategori_kln where id_kategori='$kategori'") or die(mysql_error());
	if($rown=$DB->fetch_object($qry_region))
	{
     ...
     //  $row[] ="<img src=\"$url_img/img/$image\" style=\"border: 1px solid #000; max-width:500px; max-height:500px;\" border=\"0\" />";      
       $row[]=$text_publish;
             ...
             $edit="";
        //     if($user_id!=$id) {
		$delete="<a href=\"#\" class=\"btn btn-danger\" onClick=\"confirm_delete('$url_rewrite"."proses/inbound/hdata/$id/') \"title=\"Hapus\">Hapus</a>";
//	}
            $edit="<a href=\"$url_rewrite"."content/inbound/edit/$id \" class=\"btn btn-success\" title=\"Edit\">Edit</a>";
             $publish="<a href=\"$url_rewrite"."proses/inbound/publish/$id/$text_publish\" class=\"btn btn-info\" title=\"Publish\">Publish</a>";
